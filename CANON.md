...
| Ingunn | = Yrsa (no separate age) | cult-name for Yrsa's corpse; cult *claims* an ancient martyr, ~three generations gone (~Y−100) (C-SAINT-02) |
...
| When | Event |
|---|---|
| ~Y−100 | **"Martyrdom of the Unburnt" — the CLAIMED date (hagiography, no such event).** The new church's founding legend places Ingunn "the Unburnt" [P] three generations back; canon has no martyr here. The founding relic is really Yrsa's body (Y−6), dated deep-ancient to disguise it (C-SAINT-01/02) — the false antiquity is load-bearing: a saint only ~8 years dead would be within living memory and defeat the C-SAINT-03 credibility guard. Retained only as the *fabricated* date the cult recites. |
