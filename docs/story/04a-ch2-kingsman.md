...
| **2** | **Two Vellums — the Ingunn player-knowledge beat (C-SAINT-03).** No battle. Bótólfr is introduced tending the reliquary of "Ingunn the Unburnt"; the church column halts beside Dag's open archive. The player is **shown, never told**, that the saint is Yrsa: the hagiography (ancient martyr three generations gone, unburnt-by-fire) lies on the same table as the Blót-of-Yrsa record (Y−6, thrall-girl **hanged**, *body gone from the grove by spring*). **The characters do not connect it** (dated three generations older, relocated, reliquary closed). First faint corpse-road rumor plants the Hel arc; Hvalfjorð commerce opens. | `harvest_lie_stage` (→ `suspected`), `hvalfjord_fate` (nudged), `hel_arc_stage` (→ `contact`) | `grove_testimony`, `grove_blot_witnessed` | **None** (archive / road scene — no battle, no witnessed deed). The movement here is *knowledge*, not ledger. | — (null) |
...
[STAGE: **THE JUXTAPOSITION.** Through the open shutter, the closed reliquary on the cart — *the whole, uncorrupted body a girl the fire would not take, laid by, then found* — sits forty feet from the leaf on Dag's table that reads *body gone from the grove by spring, Y−6*. The camera does not move between them; the scene does not name them together; **only the player holds both facts at once.** Dag has, in his own hand, the record that would open the reliquary — and cannot read it, because the cult dated its saint three generations older and carried her far from the grove, and grief does not do arithmetic across three fabricated generations (C-SAINT-02/03). Bótólfr, in the yard, keeps the lid shut out of reverence. Neither man will ever be closer to it than this, until Ch4.]
...
→ Merged into `00-frame.md` §3 Saga Discrepancies, rows 15–17 (the Ingunn hagiography row folded into the existing row 8).
